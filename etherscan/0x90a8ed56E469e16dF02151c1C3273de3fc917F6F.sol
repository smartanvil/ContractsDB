contract aEthereumlotteryNet {
	...
	*/
	...
	modifier onValidContract() { if (!contractEnabled) { throw; } _; }	modifier OnlyInTime() { if (game.endDate < now) { throw; } _; }	modifier OnlyWhileWait() { if (game.drawStatus != drawStatus_.Wait) { throw; } _; }	modifier OnlyWhilePrepared() { if (game.drawStatus != drawStatus_.Prepared) { throw; } _; }	modifier ReadyForPrepare() { if (game.endDate > now || game.drawStatus != drawStatus_.Wait) { throw; } _; }	modifier ReadyForDraw() { if (game.prepareDrawBlock > block.number) { throw; } _; }	modifier OnlyOwner() { if (owner != msg.sender) { throw; } _; }}