...
contract Registry is Owned {
    ...
    modifier when_unreserved(bytes32 _name) { if (entries[_name].owner != 0) return; _; }    modifier only_owner_of(bytes32 _name) { if (entries[_name].owner != msg.sender) return; _; }    modifier when_proposed(string _name) { if (entries[sha3(_name)].reverse != msg.sender) return; _; }    modifier when_fee_paid { if (msg.value < fee) return; _; }
